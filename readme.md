...
실무 예산 및 자원 제약 등으로 인해 Redis를 사용하지 못할때, Local Cache를 통해 이에 준하는 성능을 낼 수 있을까라는 의문에서 출발한다.

## 2. 테스트 시나리오

중요한 것은 Redis를 대체할 만큼의 파이프라인을 구축하면서, local Cache의 부하 분산 효과를 최대한 확보할 수 있는 방안을 설계하는 것이다.

이를 위해 고려한 사항은 아래 3가지이다.
...
```
          ...
          ┌────────┴────────┐
          ▼                 ▼
     CacheHandler       기타 Handler
```

또한 Binlog를 읽는 스트림은 하나로 이어서, LogClient에 연결하여 binlog의 이벤트를 연결하며 이 이벤트를 LogConsumer가 받아서 처리한다.
(*BinConsumer는 JVM과 1대1 대응)
